...
async def list_status_messages(session: AsyncSession) -> list[StatusMessage]:
    ...
    stmt = (
        ...
        .where(StatusMessage.project_id.is_(None))
        .order_by(StatusMessage.status)
    )
...
async def get_status_message(
    session: AsyncSession, status: str, *, project_id: int | None = None,
) -> StatusMessage | None:
    ...
    if project_id is None:
        stmt = select(StatusMessage).where(
            and_(
                StatusMessage.project_id.is_(None),
        ...
        stmt = select(StatusMessage).where(
            and_(
                StatusMessage.project_id == project_id,
                StatusMessage.status == status
            ),
        )
    result = await session.execute(stmt)
...
async def upsert_status_message(
    ...
    *,
    ...
    if existing is None:
        row = StatusMessage(
            project_id=project_id, status=status, message=message
        )
        session.add(row)
    else:
...
async def reset_status_messages(session: AsyncSession) -> list[StatusMessage]:
    ...
    for status, message in sorted(DEFAULT_MESSAGES.items()):
        ...
        if existing is None:
            row = StatusMessage(
                project_id=None, status=status, message=message)
            session.add(row)
            rows.append(row)
        else:
            existing.message = message
...
async def list_project_messages(
    ...
    stmt = (
        select(StatusMessage)
        .where(StatusMessage.project_id == project_id)
        .order_by(StatusMessage.status)
    )
    result = await session.execute(stmt)
