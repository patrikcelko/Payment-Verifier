...
async def test_create_project(auth_client: AsyncClient) -> None:
    ...
    resp = await auth_client.post("/api/projects", json={"name": "New-App", "status": "OK"})
...
async def test_create_duplicate_returns_409(auth_client: AsyncClient) -> None:
    """Test creating duplicate project returns HTTP 409"""

    await auth_client.post("/api/projects", json={"name": "Dup-App"})
    resp = await auth_client.post("/api/projects", json={"name": "Dup-App"})
    assert resp.status_code == 409


async def test_create_empty_name_returns_422(auth_client: AsyncClient) -> None:
    """Test creating project with empty name returns HTTP 422"""

    resp = await auth_client.post("/api/projects", json={"name": "", "status": "OK"})
...
async def test_delete_project(auth_client: AsyncClient) -> None:
    ...
    create_resp = await auth_client.post("/api/projects", json={"name": "DeleteMe"})
    pid = create_resp.json()["id"]
...
async def test_delete_project_not_found(auth_client: AsyncClient) -> None:
...
async def test_list_projects_sorted_by_name(auth_client: AsyncClient) -> None:
    """Test listing projects returns them sorted alphabetically by name"""

    await auth_client.post("/api/projects", json={"name": "Zebra"})
    await auth_client.post("/api/projects", json={"name": "Alpha"})
    await auth_client.post("/api/projects", json={"name": "Mango"})
    data = (await auth_client.get("/api/projects")).json()
    names = [p["name"] for p in data["projects"]]
    assert names == ["Alpha", "Mango", "Zebra"]


async def test_update_status(auth_client: AsyncClient) -> None:
    """Test updating project status returns HTTP 200 with updated status"""

    create_resp = await auth_client.post("/api/projects", json={"name": "StatusTest", "status": "OK"})
    pid = create_resp.json()["id"]
    resp = await auth_client.patch(f"/api/projects/{pid}/status", json={"status": "UNPAID"})
    assert resp.status_code == 200
    assert resp.json()["status"] == "UNPAID"


async def test_update_status_not_found(auth_client: AsyncClient) -> None:
    """Test updating status of non-existent project returns HTTP 404"""

    resp = await auth_client.patch("/api/projects/99999/status", json={"status": "OK"})
    assert resp.status_code == 404


async def test_update_status_invalid_rejected(auth_client: AsyncClient) -> None:
    """Test updating to invalid status returns HTTP 422"""

    resp = await auth_client.post("/api/projects", json={"name": "BadStatus"})
    pid = resp.json()["id"]
    resp = await auth_client.patch(f"/api/projects/{pid}/status", json={"status": "INVALID"})
    assert resp.status_code == 422


async def test_all_valid_statuses_accepted(auth_client: AsyncClient) -> None:
    ...
    for status in ("OK", "UNPAID", "PENDING", "OVERDUE", "PARTIAL", "SUSPENDED"):
        resp = await auth_client.post(
            "/api/projects",
            json={"name": f"S-{status}", "status": status},
        )
        assert resp.status_code == 201
        assert resp.json()["status"] == status


async def test_create_with_customer_fields(auth_client: AsyncClient) -> None:
...
async def test_create_with_contact_fields(auth_client: AsyncClient) -> None:
...
async def test_create_without_optional_fields_defaults_to_null(auth_client: AsyncClient) -> None:
    """Test creating project without optional fields defaults them to None"""

    resp = await auth_client.post("/api/projects", json={"name": "NoCust"})
    assert resp.status_code == 201
    data = resp.json()
...
async def test_update_project_details(auth_client: AsyncClient) -> None:
    """Test updating project details via PATCH updates specified fields"""

    resp = await auth_client.post("/api/projects", json={"name": "EditMe"})
    pid = resp.json()["id"]
...
async def test_update_project_partial_preserves_other_fields(auth_client: AsyncClient) -> None:
    ...
    resp = await auth_client.patch(f"/api/projects/{pid}", json={"customer_name": "Changed"})
    assert resp.status_code == 200
    data = resp.json()
...
async def test_update_project_contacts(auth_client: AsyncClient) -> None:
    """Test updating project contact information via PATCH"""

    resp = await auth_client.post("/api/projects", json={"name": "ContactEdit"})
    pid = resp.json()["id"]
    resp = await auth_client.patch(
        f"/api/projects/{pid}",
        json={
...
async def test_update_project_not_found(auth_client: AsyncClient) -> None:
    """Test updating non-existent project returns HTTP 404"""

    resp = await auth_client.patch("/api/projects/99999", json={"customer_name": "Ghost"})
    assert resp.status_code == 404
