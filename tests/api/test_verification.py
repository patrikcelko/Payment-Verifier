...
async def test_402_uses_project_specific_message(
    ...
    await upsert_status_message(
        session, status='UNPAID', message='Per-project msg', project_id=proj.id,
    )
...
async def test_ok_verification_creates_log(
    ...
    assert any(e.project_name == 'LogTest' and e.status_code ==
               200 for e in logs)
...
async def test_unpaid_verification_creates_log(
    ...
    logs = await list_request_logs(session)
    assert any(
        e.project_name == 'UnpaidLog' and e.status_code == 402 for e in logs
    )
