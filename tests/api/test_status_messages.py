...
async def test_list_returns_all_blocked_statuses(auth_client: AsyncClient) -> None:
    """Test listing status messages returns all blocked statuses"""

    resp = await auth_client.get("/api/status-messages")
    assert resp.status_code == 200
    statuses = {m["status"] for m in resp.json()["messages"]}
    assert {"UNPAID", "OVERDUE", "PARTIAL", "SUSPENDED"} <= statuses


async def test_update_message(auth_client: AsyncClient) -> None:
...
async def test_update_non_configurable_status_returns_404(auth_client: AsyncClient) -> None:
...
async def test_update_empty_message_returns_422(auth_client: AsyncClient) -> None:
    """Test updating with empty message returns HTTP 422"""

    resp = await auth_client.put("/api/status-messages/UNPAID", json={"message": ""})
    assert resp.status_code == 422


async def test_list_shows_stored_custom_messages(auth_client: AsyncClient) -> None:
    """Test listing merges custom messages with default messages"""

    await auth_client.put("/api/status-messages/UNPAID", json={"message": "Custom!"})
    resp = await auth_client.get("/api/status-messages")
    assert resp.status_code == 200
...
async def test_reset_restores_defaults(auth_client: AsyncClient) -> None:
    ...
    await auth_client.put("/api/status-messages/UNPAID", json={"message": "Temporary"})
    resp = await auth_client.post("/api/status-messages/reset")
    assert resp.status_code == 200
    msgs = {m["status"]: m["message"] for m in resp.json()["messages"]}
