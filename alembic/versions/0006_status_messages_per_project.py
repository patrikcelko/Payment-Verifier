...
def upgrade() -> None:
    ...
    op.create_index("ix_status_messages_project_id", "status_messages", ["project_id"])

    op.execute("CREATE UNIQUE INDEX uix_sm_global ON status_messages(status) WHERE project_id IS NULL")
    op.execute("CREATE UNIQUE INDEX uix_sm_project ON status_messages(project_id, status) WHERE project_id IS NOT NULL")
